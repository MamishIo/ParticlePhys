...
sealed class ParticleQuadtree(val position: Vector2D, val size: Vector2D, val depth: Int) {

    class Branch(position: Vector2D, size: Vector2D, depth: Int): ParticleQuadtree(position, size, depth) {
        ...
        private fun makeQuadrant(shiftx: Boolean, shifty: Boolean): Leaf {
            ...
            return Leaf(shift.add(position), Vector2D(halfw, halfh), depth + 1)
        }
    }

    class Leaf(position: Vector2D, size: Vector2D, depth: Int): ParticleQuadtree(position, size, depth) {
    ...
    fun addParticleIfTouching(particle: Particle) {
        if (touchesParticle(particle)) {
            when (this) {
                ...
                is Branch -> { quadrants.forEach { q -> q.addParticleIfTouching(particle) } }
            }
        }
    }
    ...
    fun subdivideIntoBranch(): Branch {
        return when (this) {
            is Branch -> this
            is Leaf -> {
                val newBranch = Branch(position, size, depth)
    ...
    fun mergeIntoLeaf(): Leaf {
        return when (this) {
            is Leaf -> this
            is Branch -> {
                val newLeaf = Leaf(position, size, depth)
    ...
    private fun enclosesParticle(particle: Particle): Boolean {
